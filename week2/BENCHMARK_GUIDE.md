...
状态：2026-09-24 已获准并在低负载 GPU 1 完成三次性能对照，见 LESSON_03_RESULTS.md。本文件保留复现实验的操作方法；以后运行前仍需重新确认 GPU 可用。
...
LESSON_03_RESULTS.md 已记录本次环境、命令、三个原始 JSON、数据表及限制。不要用单层教学实验声称优于生产推理引擎，不用 profiler 开启时的耗时填正式表。
